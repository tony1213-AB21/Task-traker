...
`project_type`/`work_type` 전송 (KAN-32): `projects.analytics_bucket`(프로젝트 생성/수정 UI에서 선택)과 Subtype 이름 키워드 매핑(`workTypeFromSubtype`)으로 안전 버킷만 전송합니다. 실제 프로젝트명/서브타입 원문은 보내지 않습니다. 현재 UI는 Entry를 기본값으로 즉시 생성하므로 `entry_saved`에는 대부분 생략되고, `todo_created`에서 주로 값이 실립니다 (Review Plan 4번 한계 참조).
...
- 2026-07-06: project_type/work_type 안전 버킷 전송 (KAN-32)
