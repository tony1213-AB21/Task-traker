...
`NEXT_PUBLIC_GA_MEASUREMENT_ID` 환경변수를 설정하면 루트 레이아웃이 gtag를 로드하고 이벤트가 전송됩니다 (`send_page_view: false`, 스펙의 9개 이벤트만 전송). 미설정 시 스크립트를 싣지 않으며 개발 환경에서는 `console.debug`로 payload를 확인할 수 있습니다.
